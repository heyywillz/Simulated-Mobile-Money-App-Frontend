...
export type FlowType = 'send' | 'pay_bill' | 'buy_goods' | 'cash_in' | 'airtime';
...
const BILLERS = [
  { id: 'ecg', name: 'ECG Electricity (Prepaid)', category: 'Power', icon: 'flash' as const },
  { id: 'gwcl', name: 'Ghana Water Company (GWCL)', category: 'Water', icon: 'water' as const },
  { id: 'dstv', name: 'DSTV / GOtv Ghana', category: 'TV', icon: 'tv' as const },
  { id: 'fiber', name: 'MTN Broadband Fiber', category: 'Internet', icon: 'wifi' as const },
  { id: 'starlink', name: 'Starlink Internet Africa', category: 'Internet', icon: 'globe' as const },
  { id: 'school', name: 'University / School Fees', category: 'Education', icon: 'school' as const },
];
...
export function TransactionModal({
  ...
  const [step, setStep] = useState<'input' | 'pin' | 'step_up' | 'processing' | 'receipt'>('input');
  ...
  const [depositMethod, setDepositMethod] = useState<'agent' | 'card' | 'bank'>('agent');
  ...
  const handleExecuteTransaction = async (pin?: string, authLayers: AuthLayer[] = ['pin']) => {
                ...
                {type === 'cash_in' && (
                  <>
                    ...
                    <View style={styles.depositMethodRow}>
                      {[
                        { id: 'agent', label: 'MoMo Agent', icon: 'business-outline' as const },
                        { id: 'card', label: 'Debit Card', icon: 'card-outline' as const },
                        { id: 'bank', label: 'Bank Transfer', icon: 'wallet-outline' as const },
                      ].map((m) => (
                        <TouchableOpacity
                          key={m.id}
                          style={[
