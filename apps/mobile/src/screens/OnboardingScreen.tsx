 ...
 *   - Step 5: Create 6-Digit MoMo PIN
 *   - Step 6: Confirm PIN
 *   - Step 7: Success Card
 */
...
import { compareFaceWithProfile, compareFingerprintWithEnrolled } from '@momo/shared';

type Step = 'personal' | 'photo' | 'kyc' | 'facial' | 'fingerprint' | 'password' | 'success';

const STEP_META: Record<Step, { title: string; subtitle: string; stage: number }> = {
...
export function OnboardingScreen({ onComplete, onSwitchToLogin, isLoading, error }: OnboardingScreenProps) {
  // ─── Form State ─────────────────────────────────────────────
  ...
  const [faceScanState, setFaceScanState] = useState<'idle' | 'positioning' | 'challenge' | 'analyzing' | 'passed'>('idle');
  const [faceChallenge, setFaceChallenge] = useState<'blink' | 'turn'>('blink');
  const [faceProgress, setFaceProgress] = useState(0);
  const [livenessScore, setLivenessScore] = useState(0);

  // ─── Interactive Fingerprint Sensor State ──────────────────
  const [fpState, setFpState] = useState<'idle' | 'holding' | 'analyzing' | 'passed'>('idle');
  const [fpProgress, setFpProgress] = useState(0);
  const fpHoldTimerRef = useRef<any>(null);
