...
export type BiometricMode = 'facial' | 'biometric' | 'pin';

interface BiometricModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (method: 'facial' | 'biometric') => void;
...
export function BiometricModal({
  ...
  const [currentMode, setCurrentMode] = useState<BiometricMode>(mode);
  const [progress, setProgress] = useState(0);
  const [scanState, setScanState] = useState<'idle' | 'scanning' | 'challenge' | 'analyzing' | 'passed' | 'failed'>('idle');
  const [challenge, setChallenge] = useState<'blink' | 'turn'>('blink');
  const [faceResult, setFaceResult] = useState<FaceComparisonResult | null>(null);
  const [fpResult, setFpResult] = useState<FingerprintComparisonResult | null>(null);
