/**
 * Typed endpoint functions for end-user apps.
 *
 * Wiring:
 *   - signup()  → Express POST /
...
export async function registerDevice(_profile: DeviceProfile): Promise<{ success: boolean }> {
  return { success: true };
}
...
export async function sendMoney(payload: TransactionRequest): Promise<TransactionResponse> {
  return executeTransaction('send', payload);
}

export async function cashOut(payload: TransactionRequest): Promise<TransactionResponse> {
  return executeTransaction('cash_out', payload);
}

export async function cashIn(payload: TransactionRequest): Promise<TransactionResponse> {
  return executeTransaction('cash_in', payload);
}

export async function payBill(payload: TransactionRequest): Promise<TransactionResponse> {
  return executeTransaction('pay_bill', payload);
}

export async function buyGoods(payload: TransactionRequest): Promise<TransactionResponse> {
...
function signupLocal(payload: SignupPayload): AuthResponse {
  const store = SimStore.get();
  const user = {
    id: `user_${Date.now()}`,
    fullName: payload.fullName,
...
function loginLocal(payload: AuthPayload): AuthResponse {
  const store = SimStore.get();
  let user = store.getUser();
