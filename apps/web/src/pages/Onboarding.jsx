...
const STEP_METADATA = {
  ...
  photo: { title: 'Upload Profile Photo', subtitle: 'Upload an official photo or take a live camera snapshot', stage: 2 },
  kyc: { title: 'Mobile & Ghana Card', subtitle: 'Primary wallet number & National ID', stage: 3 },
  facial: { title: 'Facial Liveness Scan', subtitle: 'Biometric mesh & identity verification', stage: 4 },
  fingerprint: { title: 'Fingerprint Biometrics', subtitle: 'Hardware biometric key enrollment', stage: 4 },
  password: { title: 'Create Account Password', subtitle: 'Set a secure secret password to protect your wallet account', stage: 5 },
...
export default function Onboarding() {
  ...
  useEffect(() => {
    isWebAuthnAvailable().then(setHasWebAuthn).catch(() => setHasWebAuthn(false))
  }, [])
  ...
  const startPhotoCamera = async () => {
    ...
    try {
      ...
      if (stream) {
        ...
        setError('Camera unavailable or permission denied. Please upload a photo file instead.')
      ...
      const tracker = new RealtimeFaceTracker(
        ...
        faceCanvasRef.current || undefined,
        {
          onFrame: (result) => {
            ...
            if (result.detected) {
              setFaceProgress((prev) => Math.min(100, Math.max(prev, result.livenessScore)))
  ...
  const handleWebAuthnEnroll = async () => {
    ...
    playBiometricSound('scan')
    try {
      const ok = await requestWebAuthnBiometric('Swipe Pay Ghana Fingerprint Enrollment')
      if (ok) {
        setFpState('passed')
        setFpProgress(100)
        playBiometricSound('success')
        ...
        setFpState('idle')
        setFpHoldError('Biometric prompt dismissed. Use the touch sensor below.')
      }
    } catch {
      setFpState('idle')
    }
  ...
  const handleFacialContinue = () => {
    if (!facialTemplate) {
      setFacialTemplate(`BIO-FACE-${Math.random().toString(36).substring(2, 8).toUpperCase()}`)
    }
    setStep('fingerprint')
  }
  ...
  const handleFingerprintContinue = () => {
    if (!fingerprintTemplate) {
      setFingerprintTemplate(`BIO-FP-${Math.random().toString(36).substring(2, 8).toUpperCase()}`)
    }
    setStep('password')
  }
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    const finalFaceTmpl = facialTemplate || `BIO-FACE-${Math.random().toString(36).substring(2, 8).toUpperCase()}`
    const finalFpTmpl = fingerprintTemplate || `BIO-FP-${Math.random().toString(36).substring(2, 8).toUpperCase()}`
        ...
        <div className="lg:col-span-5 bg-primary-800 text-white p-8 sm:p-10 flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-8">
              ...
              />
              <div>
                <h1 className="text-xl font-black tracking-tight leading-none">Swipe Pay</h1>
                <p className="text-[11px] text-red-200 font-medium tracking-wider uppercase mt-1">Wallet Creation & KYC</p>
              </div>
            </div>
            ...
            <div className="space-y-2.5 pt-6 border-t border-white/15">
              {[
                ...
                const isPassed = currentStageIndex > s.stage || step === 'success'
                const isCurrent = currentStageIndex === s.stage && step !== 'success'
                return (
                  <div key={s.stage} className="flex items-center gap-3 text-xs">
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-[11px] transition-all duration-300 ${isPassed
                    ...
                    <span className={`transition-colors ${isCurrent ? 'font-bold text-white' : isPassed ? 'text-white/80' : 'text-white/40'}`}>
                      {s.label}
                    </span>
                  </div>
                )
          ...
          <div className="max-w-md mx-auto w-full space-y-6 animate-fade-in">
            ...
            {step === 'personal' && (
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-neutral-500 mt-1">Please enter your accurate personal information for account KYC.</p>
                </div>
                    ...
                    <div className="grid grid-cols-3 gap-1.5 h-[42px] p-1 bg-neutral-100 rounded-xl">
                      {['male', 'female', 'other'].map((g) => (
                        <button
                          ...
                          type="button"
                          onClick={() => setGender(g)}
                          className={`rounded-lg text-xs font-bold capitalize transition-all ${gender === g
            ...
            {step === 'photo' && (
              <div className="space-y-5">
                <div>
                  <h3 className="text-2xl font-black text-neutral-900 tracking-tight">Upload Profile Photo</h3>
                  <p className="text-xs text-neutral-500 mt-1">
                    Upload a real, clear portrait photo of yourself or take a live camera snapshot. This photo is required for KYC compliance and Face ID cross-comparison.
                  </p>
                </div>
                ...
                <div className="p-6 bg-neutral-50 rounded-3xl border border-neutral-200 flex flex-col items-center justify-center text-center space-y-4">
                  {isTakingPhoto ? (
                    ...
                    <div className="flex flex-col items-center space-y-3">
                      ...
                      <p className="text-xs font-bold text-neutral-800">Official Profile Photo Attached</p>
                    ...
                    <div className="py-4 space-y-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-neutral-800">No profile photo uploaded yet</p>
                        <p className="text-xs text-neutral-500 max-w-xs mx-auto mt-0.5">
                          Please upload an image file from your device or take a live picture using your camera.
                        </p>
                      </div>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 text-sm font-bold rounded-2xl transition-all shadow-xs ${profilePicture
            ...
            {step === 'kyc' && (
              <div className="space-y-4">
                <div>
                  <h3 className="text-2xl font-black text-neutral-900 tracking-tight">Ghana Card & Phone</h3>
                  <p className="text-xs text-neutral-500 mt-1">Required for Mobile Money authorization and ATOD protection.</p>
                </div>
                ...
                <div>
                  ...
                  />
                  <p className="text-[10px] text-neutral-400 mt-1">Format: GHA-XXXXXXXXX-X</p>
                </div>
                ...
                <div className="p-3 bg-red-50/70 rounded-2xl border border-red-100 flex items-start gap-2.5">
                  ...
                  <p className="text-[11px] text-primary-900 leading-snug">
                    Ghana Card credentials enable instant transactions up to GH₵ 20,000 daily limit.
                  </p>
                </div>
            ...
            {step === 'facial' && (
              <div className="space-y-4 text-center">
                ...
                <h3 className="text-xl font-black text-neutral-900 tracking-tight">
                  ...
                  {faceScanState === 'scanning' && 'Hold Still — Scanning Face...'}
                  {faceScanState === 'analyzing' && 'Matching Facial Mesh Landmark Tokens...'}
                  {faceScanState === 'passed' && 'Facial Verification Confirmed!'}
                </h3>

                {/* Face Scanner Circle HUD */}
                <div className="relative mx-auto w-48 h-48 my-2 flex items-center justify-center">
                  <div
                    className={`w-44 h-44 rounded-full border-4 flex items-center justify-center relative overflow-hidden transition-all duration-300 shadow-inner ${faceScanState === 'passed'
                  ...
                  >
                    {hasCamera && cameraStream && faceScanState !== 'passed' ? (
                      ...
                      <div className="relative w-full h-full flex flex-col items-center justify-center bg-gradient-to-b from-neutral-850 to-neutral-950 text-white p-2">
                        ...
                        <span className="text-[9px] font-mono font-bold text-neutral-300 uppercase tracking-wider -mt-1">
                          {faceScanState === 'scanning' ? 'Scanning Mesh' : 'Live Camera'}
                        </span>
                      </div>
                    )}

                    {/* Animated Scanning Laser Line */}
                    {faceScanState === 'scanning' && !liveFaceResult?.detected && (
                      <div
                    ...
                    {faceScanState === 'passed' && (
                      <div className="absolute inset-0 bg-green-500/90 flex flex-col items-center justify-center text-white animate-scale-in">
                        ...
                        <span className="text-xs font-black uppercase mt-1">Verified</span>
                      </div>
                    )}
                  </div>
                  ...
                  {faceScanState === 'scanning' && (
                    <svg className="absolute inset-0 w-48 h-48 -rotate-90 pointer-events-none" viewBox="0 0 192 192">
                      <circle
                        ...
                        r="92"
                        fill="none"
                        stroke={liveFaceResult?.detected ? '#10B981' : '#8A0F13'}
                        strokeWidth="4"
                ...
                {hasCamera === false && faceScanState !== 'passed' && (
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-50 border border-amber-200 rounded-full text-[11px] font-semibold text-amber-800 animate-fade-in">
                    <span>⚡ Camera offline or denied. Simulation mode active.</span>
                    <button
                      type="button"
                ...
                <p className="text-xs text-neutral-500 max-w-xs mx-auto">
                  {faceScanState === 'idle' && 'AI-assisted face matching binds your physical biometric token to your new wallet.'}
                  {faceScanState === 'scanning' && (
                    liveFaceResult?.detected
                  ...
                  {faceScanState === 'analyzing' && 'Cross-referencing biometric mesh with Ghana Card registry...'}
                  {faceScanState === 'passed' && 'Biometric face signature successfully verified & tokenized.'}
                </p>
            ...
            {step === 'fingerprint' && (
              <div className="space-y-4 text-center">
                ...
                <h3 className="text-xl font-black text-neutral-900 tracking-tight">
                  ...
                  {isPressingFpSensor && 'Recording Ridge Pattern — Keep Holding...'}
                ...
                <div className="relative mx-auto w-44 h-44 my-2 flex items-center justify-center">
                  <button
                    ...
                    className={`w-36 h-36 rounded-3xl border-4 flex flex-col items-center justify-center relative overflow-hidden transition-all duration-300 select-none shadow-md ${fpState === 'passed'
                  ...
                  >
                    ...
                    <FingerprintIcon size={56} color={fpState === 'passed' ? '#16a34a' : isPressingFpSensor ? '#8A0F13' : '#525252'} />
                ...
                <p className="text-xs text-neutral-500 max-w-xs mx-auto">
                  {fpState === 'idle' && 'Press and hold the sensor to bind your fingerprint for one-tap transaction approvals.'}
                  {isPressingFpSensor && 'Hold finger steady until enrollment progress completes...'}
                  {fpState === 'passed' && 'Hardware biometric key generated and securely stored.'}
                </p>
            ...
            {step === 'password' && (
              <div className="space-y-4 animate-fade-in">
                <div>
                  <h3 className="text-lg sm:text-xl font-black text-neutral-900 tracking-tight">Create Account Password</h3>
                  <p className="text-xs text-neutral-500 mt-1">
                ...
                <div className="space-y-3">
                  ...
                  <div className="p-3 bg-neutral-50 rounded-xl border border-neutral-200/80 text-[11px] space-y-1.5">
                    <div className={`flex items-center gap-1.5 ${password.length >= 6 ? 'text-green-700 font-bold' : 'text-neutral-400'}`}>
                      <span>{password.length >= 6 ? '✓' : '○'}</span>
                      <span>At least 6 characters</span>
                    </div>
                    <div className={`flex items-center gap-1.5 ${password && password === confirmPassword ? 'text-green-700 font-bold' : 'text-neutral-400'}`}>
            ...
            {step === 'success' && (
              <div className="space-y-5 text-center animate-fade-in">
                ...
                <div>
                  <h3 className="text-2xl font-black text-neutral-900 tracking-tight">Wallet Account Ready!</h3>
                  <p className="text-xs text-neutral-500 mt-1">Welcome to Swipe Pay. Your mobile money wallet has been activated.</p>
                </div>
                ...
                <div className="p-4 bg-neutral-50 rounded-2xl border border-neutral-200 text-left space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <h4 className="text-sm font-bold text-neutral-900">{fullName}</h4>
                      <p className="text-xs text-neutral-500 font-mono">{phoneNumber}</p>
                  ...
                  <div className="pt-2 border-t border-neutral-200/60 grid grid-cols-2 gap-2 text-[11px]">
                    <div className="p-2 bg-white rounded-xl border border-neutral-100">
                      <span className="text-neutral-400 block font-bold uppercase text-[9px]">Starting Balance</span>
                      <span className="text-sm font-black text-primary-800 font-mono">GH₵ 5,000.00</span>
                    </div>
                    <div className="p-2 bg-white rounded-xl border border-neutral-100">
                      <span className="text-neutral-400 block font-bold uppercase text-[9px]">KYC Status</span>
                  ...
                  <div className="flex items-center gap-1.5 text-[10px] text-green-700 font-bold bg-green-50 p-2 rounded-xl border border-green-200">
                    ...
                    <span>Facial Liveness & Fingerprint Biometric Protection Enrolled</span>
                  </div>
                </div>
...
export async function action({ request, params } = {}) {
  ...
  try {
    const response = await axios.post("https://machine-learning-server-ohnz.onrender.com", formEntries, { withCredentials: true });
