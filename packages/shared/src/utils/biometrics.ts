...
// Synthesized audio feedback using Web Audio API (No external sound files required)
...
export function playBiometricSound(type: 'beep' | 'scan' | 'success' | 'fail' | 'tick') {
  try {
    ...
    // Gracefully ignore audio errors (e.g. user gesture restriction)
  }
}
...
export async function requestWebAuthnBiometric(promptText = 'Authorize with Fingerprint / Face ID'): Promise<boolean> {
  if (typeof window === 'undefined' || !window.PublicKeyCredential) {
    return false;
  }
  try {
    ...
    // If user cancelled or device not configured, return false so fallback interactive simulation is used
    return false;
  }
}

// Camera feed management with multi-tier progressive constraints
export async function requestUserMediaCamera(): Promise<MediaStream | null> {
  ...
  // Tier 2: Generic camera with ideal resolution (Fixes Windows USB webcams & laptops that reject facingMode)
  try {
    const stream = await navigator.mediaDevices.getUserMedia({
      video: {
        width: { ideal: 640 },
 ...
 * Sets muted=true and playsinline to satisfy browser autoplay security policies.
 */
