 ...
 */
export function findClosestGhanaianCity(lat: number, lng: number): PresetLocation | null {
  let closest: PresetLocation | null = null;
 ...
 * Starlink routers and satellite downlink POPs in Africa often report US (e.g., Short Mountain Arkansas, Hawthorne, Atlanta, Dallas)
 * or European ground stations because the Starlink ground gateway IP/BSSID is registered at the SpaceX gateway station.
 */
export function isStarlinkOrSatelliteUplink(city: string, region: string, country: string, lat: number, lng: number): boolean {
  const cLower = (city || '').toLowerCase();
  const rLower = (region || '').toLowerCase();
  const cntryLower = (country || '').toLowerCase();

  // Known Starlink ground station POPs that bleed into African subscriber terminals
  if (
    ...
    (cntryLower.includes('united states') && Math.abs(lat - 35.2) < 5) // Arkansas ground POP range
  ) {
    return true;
  }

  // If outside Ghana boundaries (Ghana bounding box approx: Lat 4.5 to 11.5 N, Lng -3.5 to 1.5 E)
  // while user is on local domestic network / Starlink
 ...
 */
export function setStoredLocation(loc: LocationData): void {
 ...
 * Detects and corrects Starlink satellite ground station downlinks (e.g. Arkansas POP) to the user's physical Sunyani terminal.
 */
/**
 * Reverse geocode latitude and longitude to extract exact physical city, region, and country.
 */
async function reverseGeocode(lat: number, lng: number): Promise<{ city: string; region: string; country: string }> {
 ...
 */
export async function detectExactLocation(): Promise<LocationData> {
  ...
  if (typeof navigator !== 'undefined' && 'geolocation' in navigator) {
    try {
      ...
      // Attempt to capture current public IP address
 ...
 */
export function watchExactLocation(callback: (loc: LocationData) => void): (() => void) | null {
  if (typeof navigator === 'undefined' || !('geolocation' in navigator)) {
    return null;
  }
