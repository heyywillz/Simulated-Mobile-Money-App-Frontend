  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      socket.on('admin:stats:updated', (updatedStats) => {
        setData((prev: any) => ({ ...prev, ...updatedStats }))
      })
      return () => {
        socket.disconnect()
      }
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data.flagsOverTime}>
                ...
                <XAxis
                  ...
                  tickFormatter={(val: string) => new Date(val).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}
                  interval={4}
                />
                <YAxis tick={{ fontSize: 10, fill: '#A3A3A3' }} />
                <Tooltip
                  contentStyle={{ borderRadius: '12px', border: '1px solid #E5E5E5', fontSize: '12px' }}
                  labelFormatter={(val: string) => new Date(val).toLocaleDateString('en-GB', { day: 'numeric', month: 'long' })}
                />
        ...
        <div className="overflow-x-auto">
          <table className="w-full data-table min-w-[600px]">
            ...
            <tbody className="divide-y divide-neutral-50">
              {data.topFlaggedAccounts.map((account: any) => (
